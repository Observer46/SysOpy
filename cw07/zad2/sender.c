...
void exit_cleanup(){
    for(int i=0; i < SEMAPHORE_COUNT; i++)         // Zamykamy semafory
        if (semaphores[i] != NULL && sem_close(semaphores[i]) == -1)
            fprintf(stderr,"Sender: Matko kochana, juz nawet nie ma co zbierac z tych semaforow...\n");
    ...
    if (orders != NULL && orders != (void*) -1 && munmap(orders, sizeof(orders_struct)) == -1 )         // Na wypadek zakonczenia programu gdy ktorys z pracownikow mial odwarta pamiec wspoldzielona
        fprintf(stderr,"Sender: Matko kochana, juz nawet nie ma co zbierac z pamieci wspoldzielonej...\n");
...
void send_order(){
    if ( sem_wait(semaphores[ARRAY_STATUS]) == -1 )     // Oczekiwanie na wolna tablice      
        error_exit("Sender: Nie powiodlo sie czekanie semafora ARRAY_STATUS!", 5);

    if ( sem_post(semaphores[SEND_IDX]) == -1)          // Zwiekszamy indeks kolejnej paczki do wyslania
        error_exit("Sender: Nie udalo sie zwiekszy semafora SEND_IDX!", 6);

    if ( sem_wait(semaphores[SEND_COUNT]) == -1)         // Zmniejszamy ilosc paczek do wyslania
        error_exit("Sender: Nie udalo sie zwiekszy semafora SEND_COUNT!", 7);
    ...
    int idx;
    if (sem_getvalue(semaphores[SEND_IDX], &idx) == -1)  error_exit("Sender: Nie udalo sie odczytac z semaforow indeksu paczki do wyslania!", 3);
    idx--;
    ...
    if (sem_getvalue(semaphores[WRAP_COUNT], &orders_to_wrap) == -1 || sem_getvalue(semaphores[SEND_COUNT], &orders_to_send) == -1)  
        error_exit("Sender: Nie udalo sie odczytac ilosci paczek do zapakowania/do wyslania z semaforow!",3);
    ...
    if (sem_post(semaphores[ARRAY_STATUS]) == -1 )  // Zwolnienie tablicy na wolna tablice
        error_exit("Sender: Nie udalo sie zwiekszyc semafora ARRAY_STATUS (zwolnic tablicy)!", 14);
}
...
void sigusr1_handle(int sig){
    while(1){
        ...
        if (sem_getvalue(semaphores[SEND_COUNT], &packages_to_send) == -1) {
...
int main(){
    ...
    if ( atexit(exit_cleanup) != 0 ){
        fprintf(stderr,"Sender: Nie udalo sie ustawic funkcji zamykajacej (atexit())!\n");
        exit(11);
    }
