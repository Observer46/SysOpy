...
void signal_handler(int sig, siginfo_t* sig_info, void* ucontext){
    if (sig == SIG_COUNT){
        received_signals++;
    if ( strcmp(glob_mode, "sigqueue") == 0){
        if ( sig_info -> si_code == SI_QUEUE)
            printf("Sender: Otrzymalem sygnal %d.\n",sig_info -> si_value.sival_int);
        else{
            fprintf(stderr,"Otrzymano sygnal z sigqueue w sposob bledny!\n");
            exit(5);
        }
    }
    }
...
void send_signals(pid_t catcher_PID, int signal_count, char* mode){
...
int main(int argc, char** argv){
    ...
    send_signals(catcher_PID, signal_count, sending_mode);
